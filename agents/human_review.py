...
class HumanReviewAgent:
    ...
    def present_for_review(self, post: Dict, fact_check_results: Dict) -> Dict:
...
def human_review_agent(state):
    ...
    review_result = reviewer.present_for_review(post, fact_check)
    ...
    if review_result['action'] == 'approve':
        ...
        if kg:
            post_id = kg.add_post(
                ...
                sources=[s.get('url') for s in post.get('sources', [])]
            )
            return {
